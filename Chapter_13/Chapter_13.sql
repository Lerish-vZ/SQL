...
-- 		   leading characters (at the front of the string),
-- 		   trailing characters (at the end of the string), or both
...
-- SYNTAX: 	left(string, number) and right(string, number)
...
-- tilde (~) case-sensitive match on a regular expression
-- tilde-asterisk (~*) to perform a case-insensitive match
-- To negate either one put ! in front.
-- 		exp. !~* indicates to not match a regular expression that is caseinsensitive
...
-- regexp_split_to_table(string, pattern) - splits delimited text into rows
-- regexp_split_to_array(string, pattern) - splits delimited text into an array
